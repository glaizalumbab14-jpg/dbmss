...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($price) || empty($category_id)) {
        ...
        try {
            $stmt = $pdo->prepare("
                INSERT INTO products (name, description, price, quantity, category_id, sku, status, featured, image) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([$name, $description, $price, $quantity, $category_id, $sku, $status, $featured, $imageName]);
...
<head>
    ...
    <title>Add Product with Image - Admin</title>
    <style>
...
<body>
    ...
    <div class="container">
        <div class="form-container">
            <h2 style="margin-bottom: 30px;">Product Information</h2>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="price" name="price" step="0.01" min="0" required value="<?= $_POST['price'] ?? '' ?>">
                </div>
                
                <div class="form-group">
                    <label for="quantity">Stock Quantity</label>
                    <input type="number" id="quantity" name="quantity" value="<?= $_POST['quantity'] ?? 0 ?>" min="0">
                </div>
                
                <div class="form-group">
                    <label for="sku">SKU (Product Code)</label>
                    <input type="text" id="sku" name="sku" placeholder="e.g., PROD-001" value="<?= $_POST['sku'] ?? '' ?>">
                </div>
